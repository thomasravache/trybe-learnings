...
function validationBook(req, res, next) {
    ...
    if (!valid) {
        return res.status(books_1.default.BAD_REQUEST).send(`O campo ${property} é obrigatório.`);
    }
    [valid, property] = validateValues(book);
    if (!valid) {
        return res.status(books_1.default.BAD_REQUEST).send(`O campo ${property} não pode ser nulo ou vazio.`);
    }
    next();
}
;
